...
To create our own Sankey diagram, we decided to use [Flourish Studio](https://flourish.studio/).   Flourish offers a variety of visualizations including three types of Sankey diagrams. We chose the multi-step alluvial Sankey  diagram because each flow is measured at the same time points, i.e. each wave of the survey.   
...
Each row in the dataset represents a unique flow or arrow between two waves of the study.  For example, in the above table, there were 3,000 **Never Smokers** in **Wave 1** (Rows 1 and 2).  Of these 3,000 **Never Smokers**, 2,000 remained **Never Smokers** and 1,000 became **Current Established Smokers** in **Wave 2**.  Starting in **Wave 2**, there were 2,200 **Current Established Smokers** (Rows 7 and 8).  Of these respondents, 1,800 remained **Current Established Smokers**  and 400 became **Former Smokers** in **Wave 3**. 
...
I loaded this dataset into R.  I only kept three variables: PERSONID , which is the unique ID of each respondent, and the three smoking category variables (one for each wave). 
...
Next we need to convert the "name" column into an integer.  I used the string subset command from the stringr to extract the wave number from the "name" column.  We can then drop the original name column.
...
The next step is to create two separate datasets: one for the source data and one for the target data.  We can create the source dataset by dropping the latest wave.  We then rename the value variable to "source" and the wave variable to "step_from". 
```{r dta_source}
#drop the last wave
dta_source <- dta_long %>% 
...
The rows in this dataset represent the starting point of each flow.  Consequently, we exclude the latest wave, which at the time of my internship was wave 3. 

#### Target Dataset

Next I created a "Target" dataset, which i the endpoint of each flow. We can create this dataset by dropping the first wave from the dataset.  I renamed the value column to "target" and the wave column to "step_to", which respectively represent the value and time at the end of each flow. I also renamed the PERSONID column to avoid duplication.  
...
Now we can append the Source and  Target datasets using the cbind() function. 
...
```{r}
mean(dta_sankey$PERSONID == dta_sankey$PERSONID_target)
```
...
Here we can see six respondents who were current established smokers at Wave 2 who became former smokers by Wave 3.

```{r w1_cur_est_smoker_w3_former_smoker}
dta_sankey %>%  
...
Note that some smoking transitions are impossible.  For example, an established or experimental smoker cannot become transfer back to being a non-smoker. 
...
The last step before uploading our dataset is to aggregate the data.  The final dataset will not be on the  individual-level but should represent each flow in the dataset.  We can accomplish this using the group_by() and summarize() functions. 
```{r aggregate_data}
...
Now that our data is in the shape we need, we can upload it to Flourish studio. First we can use export the data as a csv file.
...
Flourish should automatically recognize the correct columns in your dataset if you have arranged the them  in the following order: Source, Tareet, Value, Step From, Step to. You can use the control panel in the right sidebar to make any necessary corrections.  If everything looks okay, click the "Preview" button. 
...
Another thing that I noticed was that the "never smokers" flow seemed to be the most stable.  Never smokers in Wave 1 tended to stay never-smokers in Waves 2 and 3.  This probably stability is likely to due to the fact that all the respondents were over the age of 18 during the first wave of the study.  Adults who are never smokers do not tend to take up smoking later in life.  For this reason, it would also be interesting to explore the Youth survey to investigate how respondents under the age of 18 started to experiment with cigarette smokers.

We were also interested in exploring relapse rates among former established smokers over time.  From Wave 1 to Wave 2 about 12% (432 out 3,607) of former established smokers relapsed.  About the same proportion (12.4% or 521 out of 4,437 to be exact) relapsed between Waves 2 and 3. 


```{r echo=FALSE, fig.cap="Wave 1 to Wave 2 Relapse among Former Established Smokers", fig.align="center"}
...
```{r echo=FALSE,  fig.cap="Wave 2 to Wave 3 Relapse among Former Established Smokers", fig.align="center"}
...
# Conclusion
Creating a Sankey Diagram is a great way to explore transitions over time.  For example, upon viewing the Sankey diagram, it becomes clear that the never smokers category is remarkably stable over time.  I look forward to create ore Sankey diagrams to explore other longitudinal datasets.

# Next Steps

Finally, I have a few additional ideas of things to do with the PATH data.  Some of these would be building on the work that I already did during my internship and other ones would be charting new territory. 
...
### Survey Weighting {#survey_weights}
...
## Multi-state Modeling {#multi-state-modeling}
